...
# 2) DHT/Tracker discovery tuning. Keep all existing switches, but make cold-start
# pure-hash magnet discovery more deterministic and use DHT even when trackers exist.
replace_once(
...
# 3) Expose connected peer telemetry from libtorrent to Swift.
replace_once(
...
# 4) Add peer telemetry directly to torrent details. Limit to the 30 busiest peers
# and refresh at most once per second to keep the UI light on iPhone.
replace_once(
...
# 5) Normalize pasted magnet text. Empty '&dn' or '&dn=' parameters are harmless
# according to the URI concept, but some URL/parser combinations are less forgiving.
magnet_normalizer = r'''
...
# Scene URL path (Safari / share / custom URL scheme)
scene_helper = r'''
...
print("BT Mobile V0.1.1 patches applied successfully")
